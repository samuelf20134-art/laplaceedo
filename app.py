...
def rotulo_coeficiente(ordem_derivada):
    if ordem_derivada == 0: return "Coeficiente de y"
    if ordem_derivada == 1: return "Coeficiente de y′"
    if ordem_derivada == 2: return "Coeficiente de y″"
    if ordem_derivada == 3: return "Coeficiente de y‴"
    if ordem_derivada == 4: return "Coeficiente de y⁽⁴⁾"
...
with st.container():
    ...
    for i in range(ordem + 1):
        ...
        with cols_coef[i % len(cols_coef)]:
            coefs.append(st.number_input(rotulo_coeficiente(ordem_derivada), key=key, step=0.5))
...
if st.session_state.get("resultado_disponivel", False):
    ...
    try:
        t_num, y_num = solucao_numerica_geral(coefs, f_num_func, iniciais, (0, t_max))
    except Exception:
    ...
    fig.update_layout(
        ...
        xaxis=dict(title='t', showgrid=True, gridcolor='#E5E7EB',
                   zeroline=True, zerolinecolor='#9CA3AF', linecolor='#D1D5DB'),
        yaxis=dict(title='y(t)', showgrid=True, gridcolor='#E5E7EB',
                   zeroline=True, zerolinecolor='#9CA3AF', linecolor='#D1D5DB'),
